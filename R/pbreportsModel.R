...
getMockAttributeGroup <- function() {
  ...
  ag <- list(a1, a2)
  attributeGroup <-
    new("ReportAttributeGroup", id = "pbmilhouse.subread", attributes = ag)
...
getMockPlotGroup <- function() {
  ...
  p1 <-
    new("ReportPlot", id = "readlength", image = "/path/to/image.png")
  p2 <-
    new("ReportPlot", id = "accuracy", image = "/path/to/image2.png")
...
getMockReport <- function() {
  ...
  report <- new(
    "Report",
    id = "pbmilhouse_report_example",
    plotGroups = list(plotGroup),
