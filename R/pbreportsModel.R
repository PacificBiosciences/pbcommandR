...
# In the python code, a table is a relatively small
# list of values that can be displayed in Portal
# It's primarily used as a summary of computed values
# In R, this should be converted from a dataframe
...
setClass("ReportAttribute",
         representation(value="numeric",
                        id="character"))

setClass("ReportAttributeGroup",
         representation(id="character",
                        attributes="list"))
...
getMockAttributeGroup <- function() {
    # In the python code the ids are automatically
    # turned into absolute ids via the container
    # Not sure this is a great idea
    a1 <- new("ReportAttribute", id="n50", value=1234)
    a2 <- new("ReportAttribute", id="mean_readlength", value=4567)
    attributeGroup = new("ReportAttributeGroup", id="pbmilhouse.subread", attributes=c(a1, a2))
...
getMockPlotGroup <- function() {
    ...
    pg <- new("ReportPlotGroup", id=plotGroupId, plots=c(p1, p2))
    return(pg)
}
...
getMockTableGroup <- function() {
    ...
    tg <- new("ReportTableGroup", id="mytable_id", tableGroups=c(t, t2))
    return(tg)
}
...
getMockReport <- function() {
    ...
    report <- new("Report",
                  id="pbmilhouse_example",
                  plotGroups=c(plotGroup),
                  attributeGroups=c(attributeGroup),
                  tableGroups=c(tableGroup))
    return(report)
}
